import { Asset, Claimant, TransactionBuilder, } from "@stellar/stellar-sdk";
...
export function amountToStroops(amount) {
    ...
    const stroops = BigInt(whole) * 10000000n + BigInt(frac.padEnd(7, "0").slice(0, 7));
    if (negative)
...
export function stroopsToAmount(stroops) {
    ...
    return `${sign}${abs / 10000000n}.${(abs % 10000000n).toString().padStart(7, "0")}`;
}
function sameAmount(a, b) {
    // "1" and "1.0000000" are the same number of stroops.
    const toStroops = (s) => {
        const [whole = "0", frac = ""] = s.split(".");
        return BigInt(whole) * 10000000n + BigInt(frac.padEnd(7, "0").slice(0, 7));
    };
    return toStroops(a) === toStroops(b);
}
function assetOf(canonical) {
    if (canonical === "native")
        return Asset.native();
    const [code, issuer] = canonical.split(":");
    if (!code || !issuer)
        fail(`unreadable asset ${canonical}`);
    return new Asset(code, issuer);
}
function sameAsset(op, canonical) {
    const want = assetOf(canonical);
    return op.equals(want);
}
function sameAssetString(a, b) {
...
function stroopsOf(value, what) {
    ...
    if (stroops < 0n)
        fail(`${what} is negative (${value})`);
    return stroops;
}
function sameLimit(got, want) {
 ...
 */
export function verifyQuote(payload, request, expected = {}) {
 ...
 */
export function verifyTransaction(xdrBase64, payload, request, expected = {}) {
    ...
    try {
        ...
        fail(`the built transaction could not be parsed (${e.message})`);
    }
    if ("innerTransaction" in tx)
        fail("expected a plain transaction, got a fee bump");
    const expectedSource = payload.mode === "bootstrap" ? payload.channel || payload.sponsor : payload.source;
...
function verifyClassic(tx, payload, requested) {
    ...
    // account: it pays out of the funds it claims.
    {
        ...
        if (!feeOp.destAsset.isNative())
            fail("the fee must be collected in XLM");
        if (!sameAmount(feeOp.destAmount, stroopsToAmount(payload.charge_stroops))) {
            fail(`the fee is ${feeOp.destAmount} XLM, but ${stroopsToAmount(payload.charge_stroops)} was quoted`);
        }
...
function verifyOp(op, want, i, source) {
    switch (want.type) {
        case "create_account": {
            if (op.type !== "createAccount")
                fail(`operation ${i} is ${op.type}, expected createAccount`);
        ...
        case "payment": {
            if (op.type !== "payment")
                fail(`operation ${i} is ${op.type}, expected payment`);
            if (op.destination !== want.destination) {
                fail(`operation ${i} pays ${op.destination}, not ${want.destination}`);
            }
            if (!sameAsset(op.asset, want.asset))
                fail(`operation ${i} pays the wrong asset`);
        ...
        case "change_trust": {
            if (op.type !== "changeTrust")
                fail(`operation ${i} is ${op.type}, expected changeTrust`);
            const line = op.line;
        ...
        case "path_payment_strict_send": {
            ...
            if (!sameAsset(op.sendAsset, want.send_asset))
                fail(`operation ${i} sends the wrong asset`);
            if (!sameAmount(op.sendAmount, want.send_amount)) {
                fail(`operation ${i} sends ${op.sendAmount}, not ${want.send_amount}`);
            }
            if (!sameAsset(op.destAsset, want.dest_asset))
                fail(`operation ${i} buys the wrong asset`);
        ...
        case "create_claimable_balance": {
            ...
            if (!sameAsset(op.asset, want.asset))
                fail(`operation ${i} leaves the wrong asset`);
            if (!sameAmount(op.amount, want.amount)) {
            ...
            if (!dest)
                fail(`operation ${i} does not name ${want.destination} as a claimant`);
            if (!reclaim)
                fail(`operation ${i} does not name the sender as a reclaim claimant`);
            if (!samePredicate(dest.predicate, Claimant.predicateUnconditional())) {
                fail(`operation ${i}: ${want.destination} cannot claim immediately`);
            }
            if (!samePredicate(reclaim.predicate, reclaimPredicate())) {
